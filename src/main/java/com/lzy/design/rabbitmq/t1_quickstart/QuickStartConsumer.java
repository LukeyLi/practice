...
/**
 * @description:
 * 1.我在consumer和producer都创建了同一个exchange（exchange已经有的话不会重复创建，queue也是）,防止程序启动报错，因为使用一个没有创建的exchange会报错
 * 2.跑后续的demo的时候，一定要先启动consumer，后启动producer，因为只在consumer中声明了exchange和queue的绑定关系
